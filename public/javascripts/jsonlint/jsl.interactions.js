...
jsl.interactions = (function () {
    ...
    **/
    function validateOutputEnc() {
        ...
        try {
            ...
            if (result) {
                if (reformat) {
                    $('#jsonOutputEnc').val(JSON.stringify(JSON.parse(jsonVal), null, "    "));
                }
            }
	...
	function validateInputDec() {
		...
		try {
			...
			if (result) {
				if (reformat) {
					$('#jsonInputDec').val(JSON.stringify(JSON.parse(jsonVal), null, "    "));
				}
			}
		} catch (parseException) {
	...
	function validateOutputDec() {
		...
		try {
			...
			if (result) {
				if (reformat) {
					$('#jsonOutputDec').val(JSON.stringify(JSON.parse(jsonVal), null, "    "));
				}
			}
		} catch (parseException) {
    ...
    **/
    function init() {
        ...
        $('#formatEncOut').click(function () {
			validateOutputEnc();

            return false;
        });

		$('#formatDecIn').click(function () {
			validateInputDec();

			return false;
		});
		$('#formatDecOut').click(function () {
			validateOutputDec();
